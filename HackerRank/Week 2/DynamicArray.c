...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    // Initialize seqList (n empty arrays)
    int** seqList = malloc(n * sizeof(int*));
    int* seqSizes = calloc(n, sizeof(int));  // track sizes of each sequence
    int* seqCapacity = calloc(n, sizeof(int)); // track allocated capacity

    // Initialize lastAnswer and result storage
    int lastAnswer = 0;
    int* results = malloc(queries_rows * sizeof(int));
    *result_count = 0;

    for (int i = 0; i < n; i++) {
        seqList[i] = NULL; // empty arrays
    }
    ...
    // Free temporary seqCapacity and seqSizes
    free(seqSizes);
    free(seqCapacity);
