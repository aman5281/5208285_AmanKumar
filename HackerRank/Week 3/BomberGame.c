...
char** fullGrid(int r, int c) {
    ...
    for (int i = 0; i < r; i++) {
        newGrid[i] = malloc((c + 1) * sizeof(char));
        for (int j = 0; j < c; j++) newGrid[i][j] = 'O';
        newGrid[i][c] = '\0';
    }
    return newGrid;
}
...
char** bomberMan(int n, int r, char** grid, int* result_count) {
    *result_count = r;
    ...
    if (n == 1) {
        return copyGrid(r, strlen(grid[0]), grid);
    }

    if (n % 2 == 0) {
        return fullGrid(r, strlen(grid[0]));
    }

    char** first = detonate(r, strlen(grid[0]), grid);
    char** second = detonate(r, strlen(grid[0]), first);
