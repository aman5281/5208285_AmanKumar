...
char* highestValuePalindrome(char* s, int n, int k) {
    char* result = strdup(s);   // duplicate string so we can modify
    int* changed = calloc(n, sizeof(int)); // mark positions changed in step 1
    int used = 0;
    ...
    // If odd length, set middle digit to 9 if changes remain
